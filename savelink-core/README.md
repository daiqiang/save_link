...
| M 自动备份 | 3 | 全游戏检查、单游戏失败隔离、按传入上限计算未锁定保留候选与锁定排除 |
...
1. 恢复前必须先校验目标快照。
2. 目标损坏时不碰真实存档。
3. 第一版恢复不自动创建或复用当前状态快照；用户需要在恢复前自行决定是否手动创建快照。
4. 覆盖恢复采用同盘临时目录 + rename 替换，`.old` 必须保留到最终校验通过。
5. 替换或最终校验失败时换回 `.old`，`RestoreFailed { rolled_back }` 语义必须准确。
6. 存档目录缺失时，默认返回“需要用户选择”，不静默创建。
7. 选择“创建并恢复”时才创建目录并恢复。
8. 快照内容不可变，可变的只有备注和锁定状态。
9. 上层只认 `storage_key`，不得解析 store 的内部布局。
10. 当前存档已等于目标快照时，恢复是无操作：不覆盖、不创建快照、不发出虚假进度。
11. `Reason::BeforeRestore` 仅为历史数据库和云端协议兼容保留，当前版本不得主动产生。
