...
| 1.3 | 代强 | 2026-07-28 | 恢复前按需新建或复用可靠保护点，跳过空目录与目标相同场景；B 组增至 13 个，默认测试增至 67 个 |
...
- 测试：**67 个默认测试全绿**（`cargo test --no-fail-fast`），另有 J/L 两个真实百度测试默认忽略、均已按需执行通过。
...
| B 恢复 | 13 | 按需新建/复用保护点、空目录与目标相同跳过、损坏快照拒绝复用、替换恢复、回滚语义和进度顺序 |
...
1. 恢复前必须先校验目标快照。
2. 目标损坏时不碰真实存档。
3. 覆盖非空当前存档前必须有一个 `verify` 通过的回退点：优先复用同内容健康快照，否则新建 `before_restore` 保护点。
4. 需要新建保护点但创建失败时中止恢复，不覆盖真实存档；空目录无需创建空保护点。
...
11. 当前存档已等于目标快照时，恢复是无操作：不覆盖、不创建重复快照、不发出虚假进度。
