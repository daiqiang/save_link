...
env = os.environ.copy()
env["SAVELINK_TEST_DATA_DIR"] = str(run_root / "profile-b")
...
while time.time() < deadline:
    try:
        with urllib.request.urlopen("http://127.0.0.1:9231/json", timeout=1) as response:
...
if not targets:
    process.terminate()
    raise RuntimeError("profile B WebView2 debug target did not appear")
