...
| 1.6 | 代强 | 2026-07-28 | 同步恢复前智能保护点策略和 67 个默认测试 |

## 一句话结论

SaveLink 当前已经不是空原型，而是一个可运行、可打包、可用测试数据验收的本地 Windows 桌面 MVP。核心闭环已经跑通：添加游戏存档目录 → 创建快照 → 查看时间线 → 修改备注/锁定/删除快照 → 恢复指定版本 → 恢复前智能保护 → 设置页查看/打开数据目录。

MVP 已完成正式回归验收，`TC-17` 移除游戏不删除真实存档和 `TC-41` 启动自检清理残留均已补测通过。恢复链路已增加小文件/中文路径回归测试，并把恢复后校验加强为 `content_hash + file_count + total_size`。

云同步已完成真实百度双设备完整闭环：设备 A 上传 zip + `.ok`，设备 B 只读发现、下载、双重校验、接收至独立仓库，再绑定本机假存档目录并由用户手动恢复。恢复文件与本机目标快照 SHA-256 一致。接收不会带入设备 A 路径，绑定不会自动恢复。core 当前 67 个默认测试全绿，J/L 两个真实联网测试均已按需运行通过。
...
- 当前已等于目标时不执行覆盖；空目录不创建空保护点。
- 已有健康同内容快照时复用，只有未保护变化才创建 `before_restore` 保护点。
- 需要新建保护点但创建失败时中止恢复，不覆盖真实存档。
...
4. “绑定 -> 手动恢复”已用假目录验收通过，恢复内容与目标快照哈希一致；下载、绑定和恢复保持三个独立动作。恢复前智能保护策略由 13 个 B 组测试覆盖。
