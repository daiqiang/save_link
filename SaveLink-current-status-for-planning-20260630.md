...
| 1.5 | 代强 | 2026-07-23 | 同步真实双电脑普通账号验收结论，并增强快照上传按钮可发现性 |
...
云同步已完成真实百度双设备完整闭环：设备 A 上传 zip + `.ok`，设备 B 只读发现、下载、双重校验、接收至独立仓库，再绑定本机假存档目录并由用户手动恢复。恢复前自动备份已生成，恢复文件与本机目标快照 SHA-256 一致。接收不会带入设备 A 路径，绑定不会自动恢复。core 当前 64 个默认测试全绿，J/L 两个真实联网测试均已按需运行通过。
...
4. “绑定 -> 手动恢复 -> 恢复前自动备份”已用假目录验收通过，恢复内容与目标快照哈希一致；下载、绑定和恢复保持三个独立动作。
5. 两台真实电脑与普通百度账号验收已通过；继续长期自用观察，并推进故障回归、账号解绑、Token 安全存储和正式发布体验。
