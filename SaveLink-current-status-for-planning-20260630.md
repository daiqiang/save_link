...
云同步已完成无网络纵向闭环及真实百度双设备快照传输：设备 A 可上传 zip + `.ok`，设备 B 可只读发现、下载、双重校验并接收到独立 `repository`/SQLite。发现阶段不创建本机游戏，接收成功后才创建未绑定游戏，且不会带入设备 A 的真实存档路径；绑定前创建和恢复均禁用。core 当前 64 个默认测试全绿，J/L 两个真实联网测试均已按需运行通过。
...
- 云同步：Fake 与真实百度双设备传输闭环均已完成；待总规划确定设备 B 的目录绑定交互，并用假存档目录验收绑定后的安全恢复。
...
我建议不要继续扩展云端协议或零散添加 UI。百度网盘 API 可行性、云端数据边界、协议 v1 和真实双设备传输闭环均已有结论，应回到产品层决定设备 B 接收后的绑定与恢复体验。
...
3. 下一步先确定用户如何为云端接收的游戏绑定本机存档目录，以及绑定前后应展示哪些风险提示。
4. 交互确定后，只用假存档目录执行一次“绑定 -> 现有安全恢复 -> 恢复前自动备份”验收；下载和恢复继续保持为两个独立动作。
