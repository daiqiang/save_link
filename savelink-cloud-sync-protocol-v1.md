...
| 1.4 | 代强 | 2026-07-16 | 记录正式客户端单快照 zip/.ok 真实上云验收，不改变协议 v1 语义 |
...
2026-07-16 正式客户端已通过 `CloudSyncService` 将一条本机快照真实上传为同 ID 的 `.zip` 与 `.ok`，并在 SQLite 记录 `uploaded`；上传中、成功提示和绿色勾选均通过实机验收。下一阶段只扩展发现与下载，不修改 v1 对象格式。
