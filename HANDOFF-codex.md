...
- 设备 B 已使用假目录完成绑定和手动恢复；SQLite 生成恢复前自动备份，恢复文件与下载到本机仓库的目标快照 SHA-256 一致。
- 2026-07-23 代强和朋友已用两台真实电脑、非开发者普通百度账号跑通授权、上传、发现、下载、绑定、恢复及恢复前备份；普通用户无需创建百度应用，也无需预先打开百度网盘客户端。
