...
| 1.7 | 代强 | 2026-07-28 | 同步恢复前智能保护点策略、RestoreOutcome 状态和 67 个默认测试 |
...
```text
添加游戏 -> 选择存档目录 -> 创建快照 -> 时间线 -> 恢复 -> 恢复前智能保护
```
...
- 2026-07-28 恢复链路改为智能保护：当前已等于目标时不操作，空目录不创建空保护点，已有健康同内容快照时复用，只有未保护变化才新建 `before_restore` 保护点。

当前客观状态：

- `savelink-core`：67 个默认测试全绿，其中 B 组 13 个覆盖恢复事务和智能保护，G/H/I/K 组 27 个覆盖云基础设施、Fake 双设备闭环、百度 HTTP 契约、OAuth 和 Token 刷新；J/L 两个默认忽略的真实百度测试均已按需执行通过。
