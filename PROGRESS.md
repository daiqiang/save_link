...
> 最后更新：2026-07-08（本地 MVP + 正式回归验收 + 恢复校验增强 + 云同步草案）
...
- 云端：百度网盘/WebDAV/NAS；主线改为云端目录结构 + 增量同步。只同步 SaveLink 的共享快照事实和云端元数据，不同步真实存档目录，也不把某台电脑的 `savelink.db` 整库原样上云。
...
- 云同步草案：`savelink-cloud-sync-model-protocol-draft.md`
