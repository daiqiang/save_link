...
> 最后更新：2026-08-10（Steam 自动发现与多存档目录已接入；Elden Ring 父子路径重叠已修复并通过自动回归）
...
- Elden Ring 父子路径重叠已修复：末尾 `<storeUserId>` 只接受目录，Manifest 结果会收敛父子路径；手动添加/编辑及创建/恢复前也会拒绝重叠目录。真实绿色版候选复验可随下一次打包补做。
...
- 游戏识别已使用随包 Ludusavi Manifest SQLite 数据库接入 Steam 自动发现；Elden Ring 父子候选问题已修复，后续补一台真实多 Steam 游戏库机器及修复后绿色版候选验收。
