...
> 最后更新：2026-09-01（云端发现性能与关闭重开并发问题已修复；排队方案仍记录在下个版本）

---

## 一、现在是什么状态

SaveLink 当前是 v0.5.0 发布候选的 Windows 桌面应用，历史版本 v0.1.0 至 v0.4.0 已公开发布。动态发现阶段 1 至 5 已完成：待配置游戏可由 SaveLink 启动并监测，退出后由用户确认候选目录，再接管现有自动备份。`Hole Is Mine` 完整闭环、RUNE《暗黑地牢》目录归并和 `HowManyDudes` 主存档定位均已通过真实绿色版验证。云端存档的重复请求冲突、长时间读取和失败假空问题也已完成修复与真实百度只读回归。
