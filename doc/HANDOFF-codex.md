...
| 1.11 | Codex | 2026-08-10 | 同步 Steam 自动发现、多存档目录、90 个默认测试和真实绿色版验收；记录 Elden Ring 配置父目录误入保护路径的问题 |

> 角色约定：总规划会话负责方向、范围和验收判断；本地开发会话负责按文档实现、验证、打包。
> 开工前先读完本文档 + `PROGRESS.md`，再动代码。
> 最后更新：2026-08-10（Steam 自动发现与多存档目录已接入并完成主链路验收，保留一个候选路径分类问题）
...
- 当前已知问题：Elden Ring 的纯配置文件 `GraphicsConfig.xml` 归一到父目录后，该父目录仍会重复进入保护路径，并与真实数字用户存档目录形成父子重叠。修复前不能把该游戏的自动候选分类视为完全通过。

当前客观状态：

- `savelink-core`：90 个默认测试全绿，其中 N 组 3 个覆盖 Steam 多库发现、精确文件规则归一和数据库缺失报错，O 组 4 个覆盖多目录指纹、存储往返、整体恢复和旧库迁移。J/L 两个默认忽略的真实百度测试均已按需执行通过。
...
Steam 候选发现和多目录主链路已接入并通过自动测试与真实绿色版验收。扫描和快照会合并所有根目录；仓库与云端归档保留根目录边界；恢复会替换每个根目录，任一路径失败时回滚全部已替换目录；编辑、恢复确认、快照详情、设备 B 绑定和移除确认均展示完整路径列表。当前剩余问题是纯配置精确文件归一到父目录后可能误入保护候选，已在 Elden Ring 上稳定复现。

### P7：自动化 / 游戏识别 / 云端

- v0.2.0 自动备份已进入当前 P0：全局开关默认开启；启动时立即检查，之后每 10 分钟检查变化，不依赖游戏退出；每个游戏保留最近 30 个未锁定记录，锁定记录在此之外且不设数量上限；最终同步清理本地与百度网盘旧记录。
- 游戏识别已使用随包 Ludusavi Manifest SQLite 数据库接入 Steam 自动发现；需要继续修复纯配置父目录误入保护候选，并补一台真实多 Steam 游戏库机器的验收。
