...
| 1.9 | 代强 | 2026-08-05 | 同步 v0.1.0 公开发布、双仓库结构、发布附件偏差和发布后 P0 |

> 角色约定：总规划会话负责方向、范围和验收判断；本地开发会话负责按文档实现、验证、打包。
> 开工前先读完本文档 + `PROGRESS.md`，再动代码。
> 最后更新：2026-08-05（v0.1.0 发布后状态）

---

## 一、现在是什么状态

SaveLink 当前是一个已公开发布 v0.1.0、可运行、可打包、可用测试数据验收的 Windows 桌面应用。
...
v0.1.0 已经公开发布，当前不要直接扩展 v0.2。权威清单位于 `PROGRESS.md` 的“v0.1.0 已发布与 v0.1.x 发布后收口（当前 P0）”：
...
仅在发布后 P0 风险得到控制后重新排序。当前候选包括游戏/存档目录自动识别、自动快照与保留策略、更多云盘或 NAS、真实恢复进度事件，以及低频的百度账号身份显示、解绑和安全切换等，不提前承诺顺序。
