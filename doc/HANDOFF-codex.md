...
| 1.27 | Codex | 2026-09-03 | 吸收七轮评审并冻结设计：补齐仓库 ID 不一致、`uinfo.uk` 核验分类和跨账号同内容接收边界 |
| 1.28 | Codex | 2026-09-05 | 暂停 v0.6.0 实施，切换到 v0.5.1 发布收口并统一版本元数据 |
...
SaveLink 当前进入 v0.5.1 发布收口，版本元数据已统一，core 150 项、Tauri 40 项和前端生产构建均通过；正式包仍须从最终发布提交重新生成。v0.6.0 云冲突治理业务实现尚未开始，暂时停止按长篇技术方案直接开发，待用户重新确定产品范围。动态发现阶段 1 至 5 已完成：待配置游戏可由 SaveLink 启动并监测，退出后由用户确认候选目录，再接管现有自动备份。`Hole Is Mine` 完整闭环、RUNE《暗黑地牢》目录归并和 `HowManyDudes` 主存档定位均已通过真实绿色版验证。
...
- 当前发布候选：`v0.5.1`；已发布版本：`v0.1.0`、`v0.2.0`、`v0.3.0`、`v0.4.0`。程序识别、动态发现、本地一致性检查和云端增量读取修复均纳入 v0.5.1。
...
- v0.6.0 当前权威设计为 `SaveLink-v0.6.0云任务调度与冲突治理方案.md`：先引入内存协调器和应用级共享网络 Runtime，再将上传、发现、下载、metadata 和删除全部入队；Runtime 共享 HTTP Client 和限流器，Token 按真实账号分路径保存，每个任务创建无跨任务目录缓存并绑定 generation 的 Store。一个本机快照只归属一个百度账号，跨账号同内容也不能建立第二条关系；`uinfo.uk` 是账号身份，远端 manifest ID 只允许首次绑定且不一致时停止同步。跨设备删除使用不可变 tombstone 并受新鲜远端检查保护，且须通过 10,000 条下的单对象点查门槛。方案已完成七轮评审并冻结，下一步按阶段实现和验收。
