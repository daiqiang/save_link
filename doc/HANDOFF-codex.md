...
| 1.9 | 代强 | 2026-08-05 | 同步 v0.1.0 发布与并行待办；记录 v0.2.0 自动备份及锁定记录不计入 30 条上限的产品决策 |

> 角色约定：总规划会话负责方向、范围和验收判断；本地开发会话负责按文档实现、验证、打包。
> 开工前先读完本文档 + `PROGRESS.md`，再动代码。
> 最后更新：2026-08-05（v0.2.0 自动备份设计阶段）

---

## 一、现在是什么状态

SaveLink 当前是一个已公开发布 v0.1.0、可运行、可打包、可用测试数据验收的 Windows 桌面应用；项目已跳过 v0.1.1，当前主线进入 v0.2.0 自动备份设计。
...
权威方案位于 `SaveLink-v0.2.0自动备份方案.md`。当前已确认：启动立即检查，之后每 10 分钟检测变化；变化后创建快照并立即上云；每个游戏保留最近 30 个未锁定记录，锁定记录不计入 30 个且不设数量上限；多设备冲突由用户选择。手动快照是否计入 30 个仍待讨论。

任何自动快照实现都必须满足：内容不变不创建；写入不稳定、文件占用或读取失败时不登记半成品；本地快照校验成功后才能上传；自动任务不得在后台覆盖多设备冲突。
...
- v0.2.0 自动备份已进入当前 P0：启动时立即检查，之后每 10 分钟检查变化，不依赖游戏退出；每个游戏保留最近 30 个未锁定记录，锁定记录在此之外且不设数量上限。
