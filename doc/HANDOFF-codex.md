...
> 最后更新：2026-08-31（云端任务占用提示已修正；排队方案已记录但尚未实现）

---

## 一、现在是什么状态

SaveLink 当前是 v0.5.0 发布候选的 Windows 桌面应用，历史版本 v0.1.0 至 v0.4.0 已公开发布。动态发现阶段 1 至 5 已完成：待配置游戏可由 SaveLink 启动并监测，退出后由用户确认候选目录，再接管现有自动备份。`Hole Is Mine` 完整闭环、RUNE《暗黑地牢》目录归并和 `HowManyDudes` 主存档定位均已通过真实绿色版验证。
...
- `savelink-app`：Tauri 40 个测试全绿，React `npm run build` 和严格 Clippy 通过；启动绑定、防重、仅启动、原生监听、候选确认、首份自动快照、快照显示编号、云端元数据清理保护及云任务占用类型均有回归覆盖。
