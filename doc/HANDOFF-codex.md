...
权威方案位于 `SaveLink-v0.2.0自动备份方案.md`。当前实现：全局开关默认开启；启动立即检查，之后每 10 分钟检测变化；自动快照校验后立即上云；手动、自动和云端接收的未锁定记录统一计入可配置上限，默认 10、范围 1 到 100；锁定记录不计入且数量不限；旧记录按云端优先顺序与本地联合清理，云删除失败保留本地等待重试。
