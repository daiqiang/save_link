...
SaveLink 当前是一个已公开发布 v0.1.0、正在准备发布 v0.3.0 的 Windows 桌面应用。v0.3.0 包含已经验收的自动备份、自动上云、30 条联合清理、跨设备时间兼容、Steam 自动发现及多存档目录支持，版本元数据已统一为 `0.3.0`。

核心闭环已经可用：

```text
Steam 自动发现或手动添加 -> 绑定一个或多个存档目录 -> 创建快照 -> 时间线 -> 恢复
```
...
- `savelink-core`：93 个默认测试全绿，其中 N 组 5 个覆盖 Steam 多库发现、精确文件规则归一、数据库缺失报错、Elden Ring 同级配置文件和嵌套规则收敛，O 组 5 个覆盖多目录指纹、存储往返、整体恢复、旧库迁移和重叠来源拒绝。J/L 两个默认忽略的真实百度测试均已按需执行通过。
- `savelink-app`：Tauri 自动上传状态选择测试 2 个全绿，React `npm run build` 通过；自动备份、真实百度、时间修复、Steam 自动发现及多目录主流程均已验收。npm、Cargo、Tauri 和打包元数据均为 `0.3.0`，正式绿色版已生成并完成版本、内容和 SHA-256 核验。
...
- 当前已发布版本：`v0.1.0`；当前待发布代码版本：`v0.3.0`。
