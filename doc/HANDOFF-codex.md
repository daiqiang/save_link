...
| 1.18 | Codex | 2026-08-25 | 完成动态发现阶段 1、2：待配置模型、普通 EXE/Steam 启动绑定、防重、旧数据修复及窗口回归 |

> 角色约定：总规划会话负责方向、范围和验收判断；本地开发会话负责按文档实现、验证、打包。
> 开工前先读完本文档 + `PROGRESS.md`，再动代码。
> 最后更新：2026-08-25（动态发现阶段 1、2 已完成；下一步实施正式目录监听、PID 生命周期和候选分析）

---

## 一、现在是什么状态

SaveLink 当前是一个已公开发布 v0.1.0、v0.2.0、v0.3.0 和 v0.4.0 的 Windows 桌面应用。发布后主线已完成游戏程序识别，以及动态发现阶段 1、2：Windows 监听 POC、待配置游戏模型、普通 EXE/Steam 启动绑定和跨入口防重。下一步从阶段 3 开始迁入正式监听器并接游戏启动、PID 生命周期和候选分析。
...
- 待配置游戏可在没有存档目录时先登记，但创建、恢复、上传均被拒绝，自动备份会正常跳过；首页明确显示当前不会创建备份。“启动游戏并查找存档”在阶段 3 接通前保持禁用。
...
- `savelink-app`：Tauri 10 个测试全绿，React `npm run build` 和 `cargo fmt -- --check` 通过；Steam 启动绑定持久化、AppID 防重及旧普通游戏 EXE 补绑已完成隔离窗口回归。
...
权威设计是 `SaveLink游戏启动与存档动态发现方案.md`。阶段 1 原生 Windows POC 和阶段 2 待配置/启动绑定已经完成。当前模型用仅本机的 `launch_binding` 保存普通 EXE 或受控 Steam 启动方式，不进入云协议；Steam AppID、EXE/安装目录和存档路径共同参与跨入口防重。
...
下一步只做阶段 3：把 POC 的 `ReadDirectoryChangesW` 窄接口迁入正式工程，建立全局单发现会话，接入受控游戏启动、PID 退出/5 秒宽限期、主动停止、事件聚合和候选结果。目录确认与首次自动备份接管留在阶段 4。
