...
| 1.27 | Codex | 2026-09-03 | 吸收五轮评审：采用任务级 Store、真实账号身份与请求级 generation 租约，并补新鲜远端查询和墓碑点查规模门槛 |

> 角色约定：总规划会话负责方向、范围和验收判断；本地开发会话负责按文档实现、验证、打包。
> 开工前先读完本文档 + `PROGRESS.md`，再动代码。
> 最后更新：2026-09-03（v0.6.0 云任务方案完成五轮评审修订，等待最终复审后开工）

---

## 一、现在是什么状态

SaveLink 当前 v0.5.0 基线已经完成验证，v0.6.0 处于云冲突治理最终复审阶段，业务实现尚未开始。动态发现阶段 1 至 5 已完成：待配置游戏可由 SaveLink 启动并监测，退出后由用户确认候选目录，再接管现有自动备份。`Hole Is Mine` 完整闭环、RUNE《暗黑地牢》目录归并和 `HowManyDudes` 主存档定位均已通过真实绿色版验证。云端存档的重复请求冲突、长时间读取和失败假空问题已完成修复与真实百度只读回归；五轮设计评审意见已经吸收，最终复审通过后以统一云任务协调器替代拒绝式全局互斥。
...
- v0.6.0 当前权威设计为 `SaveLink-v0.6.0云任务调度与冲突治理方案.md`：先引入内存协调器和应用级共享网络 Runtime，再将上传、发现、下载、metadata 和删除全部入队；Runtime 共享 HTTP Client 和限流器，Token Provider 按真实账号和 generation 隔离，每个任务创建无跨任务目录缓存并绑定 generation 的 Store。每次 HTTP 请求都必须取得匹配凭据租约，旧任务不能操作新账号，旧账号可靠义务也不能在新账号恢复。跨设备删除使用不可变 tombstone 并受新鲜远端检查保护，且须通过 10,000 条下的单对象点查门槛。方案已完成五轮评审修订，最终复审前不要开始业务实现。
