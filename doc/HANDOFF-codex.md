...
SaveLink 当前是一个已公开发布 v0.1.0、v0.2.0、v0.3.0 和 v0.4.0 的 Windows 桌面应用。发布后主线新增从 Windows 快捷方式、EXE 或安装目录识别游戏及存档候选，代码、自动测试和真实学习版样本只读验收均已完成。无可靠 AppID 且名称不能精确匹配时会返回空结果，由手动添加兜底。
...
- `savelink-core`：109 个默认测试全绿，其中 N 组新增 7 个程序识别测试，覆盖 AppID、精确名称兜底、目录配置、未知程序、名称前缀拒绝、缺失路径和真实 Windows `.lnk`；J/L 已按需执行通过，Q5 真实 DeSmuME 测试默认忽略且已执行通过。
- `savelink-app`：Tauri 自动上传状态选择测试 2 个全绿，React `npm run build` 通过；真实学习版样本的快捷方式、EXE 和安装目录窗口验收已完成，均未启动程序或修改真实文件。
...
DeSmuME 代码主链路已完成：扫描模拟器和 ROM、解析 Header/SHA-256、精确选择 `.dsv`、保存 ROM 身份、SQLite 迁移、云端身份兼容、添加/重新绑定 UI，以及目标文件级快照和回滚恢复。ROM 目录扫描已改为带 canonical 路径去重的迭代遍历，避免 junction/目录链接环递归；ROM SHA-256 的 1 MiB 流式缓冲区已移到堆上，避免 Tauri 工作线程在首次读取 ROM 时栈溢出。当前默认 109 个 core 测试、前端构建和真实样本 Q5 只读发现均通过。
