...
> 最后更新：2026-09-02（v0.5.0 基线完成；v0.6.0 进入云任务调度与冲突治理主线）

---

## 一、现在是什么状态

SaveLink 当前 v0.5.0 基线已经完成验证，v0.6.0 进入云冲突治理设计与实现阶段。动态发现阶段 1 至 5 已完成：待配置游戏可由 SaveLink 启动并监测，退出后由用户确认候选目录，再接管现有自动备份。`Hole Is Mine` 完整闭环、RUNE《暗黑地牢》目录归并和 `HowManyDudes` 主存档定位均已通过真实绿色版验证。云端存档的重复请求冲突、长时间读取和失败假空问题已完成修复与真实百度只读回归；下一步以统一云任务协调器替代拒绝式全局互斥。
...
- v0.6.0 当前权威设计为 `SaveLink-v0.6.0云任务调度与冲突治理方案.md`：先引入内存协调器和应用级共享百度 Runtime，再将上传、发现、下载、metadata 和删除全部入队；同快照 FIFO 串行、不同快照有限并发，SQLite 业务状态负责跨重启恢复。
