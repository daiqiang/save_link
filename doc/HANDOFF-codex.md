...
- 动态发现使用原生 `ReadDirectoryChangesW` 多根监听、PID 生命周期和 5 秒退出宽限期；会话结束补查只进入游戏身份相关子树、只收集本轮时间窗口文件并设硬上限。候选默认不选择，确认前不会写游戏配置。
...
- `savelink-app`：Tauri 29 个测试全绿，React `npm run build` 和严格 Clippy 通过；启动绑定、防重、原生监听、候选确认及首份自动快照均有回归覆盖。
