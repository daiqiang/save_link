...
| 1.9 | 代强 | 2026-08-05 | 同步 v0.1.0 发布；完成 v0.2.0 全局开关、定时检查、自动快照上云、云删除状态迁移及 30 条联合清理代码 |

> 角色约定：总规划会话负责方向、范围和验收判断；本地开发会话负责按文档实现、验证、打包。
> 开工前先读完本文档 + `PROGRESS.md`，再动代码。
> 最后更新：2026-08-05（v0.2.0 自动备份第一阶段代码完成，待实机验收）
...
- `savelink-core`：75 个默认测试全绿，其中 B 组 14 个覆盖直接恢复、无恢复前自动快照、事务回滚和历史数据兼容；G/H/M 组新增保护旧库迁移、联合删除失败重试、自动检查和保留候选。J/L 两个默认忽略的真实百度测试均已按需执行通过。
- `savelink-app`：Tauri 自动上传状态选择测试 2 个全绿，React `npm run build` 通过；`build-portable.bat --no-open` 已生成 release 绿色版/ZIP，尚未完成真实窗口人工验收。开发包仍显示 0.1.0，不要上传覆盖公开 Release。
...
下一步不得继续盲目扩功能，应先打绿色版验证设置开关、启动即时检查、10 分钟周期、时间线刷新和 30 条清理，再做真实百度断网、授权失效、限流及删除失败回归。自动任务不得弹授权页或后台覆盖多设备冲突。
