...
| 1.27 | Codex | 2026-09-03 | 吸收首轮评审：删除取代未运行任务，增加 tombstone、上传中断恢复分流、ignored 兼容和完整 SQLite CHECK 迁移 |

> 角色约定：总规划会话负责方向、范围和验收判断；本地开发会话负责按文档实现、验证、打包。
> 开工前先读完本文档 + `PROGRESS.md`，再动代码。
> 最后更新：2026-09-03（v0.6.0 云任务方案完成首轮评审修订，等待复审后开工）

---

## 一、现在是什么状态

SaveLink 当前 v0.5.0 基线已经完成验证，v0.6.0 处于云冲突治理设计复审阶段，业务实现尚未开始。动态发现阶段 1 至 5 已完成：待配置游戏可由 SaveLink 启动并监测，退出后由用户确认候选目录，再接管现有自动备份。`Hole Is Mine` 完整闭环、RUNE《暗黑地牢》目录归并和 `HowManyDudes` 主存档定位均已通过真实绿色版验证。云端存档的重复请求冲突、长时间读取和失败假空问题已完成修复与真实百度只读回归；复审通过后，下一步以统一云任务协调器替代拒绝式全局互斥。
...
- v0.6.0 当前权威设计为 `SaveLink-v0.6.0云任务调度与冲突治理方案.md`：先引入内存协调器和应用级共享百度 Runtime，再将上传、发现、下载、metadata 和删除全部入队；同快照普通任务 FIFO，删除取代未运行或等待中的任务，运行请求结束后再删。跨设备删除使用不可变 tombstone，SQLite 业务状态负责跨重启恢复。方案已完成首轮评审修订，复审前不要开始业务实现。
