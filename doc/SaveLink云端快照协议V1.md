...
| 1.9 | Codex | 2026-09-03 | 同步 v0.6.0 两轮评审修订：删除先发布逐快照 tombstone，上传、接收、metadata 和恢复检查均受其约束；实现前仍保持 v0.5.0 行为 |
...
v0.6.0 目标顺序调整为：创建或确认 `snapshot-tombstones/{snapshot_id}.json`，再删除独立元数据对象、`.ok`、`.zip` 和本机快照。tombstone 长期保留；所有 v0.6+ 客户端上传前后、接收下载前与本机提交前、metadata 写入前后、上传恢复补记成功前都必须检查。缓存命中的不可变 tombstone 不重复下载 JSON；旧客户端不理解该扩展，因此发布说明必须要求同仓库设备一起升级。详细竞态规则和规模门槛以 v0.6.0 权威设计为准。
