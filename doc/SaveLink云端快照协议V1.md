...
| 1.9 | Codex | 2026-09-03 | 同步 v0.6.0 修订设计：删除先发布逐快照 tombstone，再清理 metadata、`.ok` 和 ZIP；实现前仍保持 v0.5.0 行为 |
...
v0.6.0 目标顺序调整为：创建或确认 `snapshot-tombstones/{snapshot_id}.json`，再删除独立元数据对象、`.ok`、`.zip` 和本机快照。tombstone 长期保留，所有 v0.6+ 客户端上传前后必须检查；旧客户端不理解该扩展，因此发布说明必须要求同仓库设备一起升级。
