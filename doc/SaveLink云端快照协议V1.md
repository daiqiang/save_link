...
| 1.9 | Codex | 2026-09-03 | 同步 v0.6.0 四轮评审修订：共享网络组件但不共享目录状态，所有远端存在性操作使用新鲜结果；实现前仍保持 v0.5.0 行为 |
...
v0.6.0 目标顺序调整为：创建或确认 `snapshot-tombstones/{snapshot_id}.json`，再删除独立元数据对象、`.ok`、`.zip` 和本机快照。tombstone 长期保留；所有 v0.6+ 客户端上传前后、接收下载前与本机提交前、metadata 写入前后、上传恢复补记成功前都必须检查。旧仓库缺少该可选目录等价于空集合；缓存只有在本地与远端修改时间都存在且相等时命中。并发删除只比较 schema、对象类型和 repository/game/snapshot 身份，审计字段采用远端先创建版本。合法 tombstone 缓存后，本机副本仍可编辑，但不再产生 metadata 云任务。旧客户端不理解该扩展，因此发布说明必须要求同仓库设备一起升级。详细竞态规则和规模门槛以 v0.6.0 权威设计为准。
...
v0.6.0 起，上述接口的远端存在性语义必须是新鲜的：`list_directory/stat_file` 每次查询后端，`get_file/delete_file` 不得根据旧父目录缓存判断目标，`put_file(CreateOnly)` 以服务端创建结果为权威。Store 不保存无 TTL 的跨任务 `directory_cache`；SQLite 缓存只用于跳过未变化对象的内容下载，不能替代目录枚举。共享范围限制为 HTTP Client、Token Provider 和限流器，每个任务创建短生命周期 Store，目录确保集合也仅在任务内有效。
...
清除授权或重新授权时必须使旧 Runtime generation 失效并重建网络运行时；旧任务结果不得提交到新账号状态，任何目录和目录确保状态都不能跨账号继承。
