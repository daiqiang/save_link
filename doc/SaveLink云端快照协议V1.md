...
| 1.9 | Codex | 2026-09-03 | 同步 v0.6.0 七轮评审最终收口：补齐仓库 ID 不一致、`uinfo.uk` 身份分类和跨账号同内容接收边界；实现前仍保持 v0.5.0 行为 |
...
- `remote_repository_id` 为空时只能原子首次绑定；已绑定值相同才能继续；已绑定值与远端 manifest 不同时必须保留原值并报告 `repository_mismatch`，不得覆盖、重新初始化仓库或执行该账号的目录刷新、上传、接收、metadata、删除及恢复。
...
- 来源账号等于 `cloud_owner_account_id` 且内容字段完全一致：视为幂等成功，更新该账号同步状态，不重复写入。
- `cloud_owner_account_id` 为空且内容字段完全一致：只在最终 SQLite 事务成功将 owner 原子认领为来源账号后视为幂等成功；并发认领失败则不新增同步关系。
- owner 属于另一账号：无论内容是否一致都返回 `snapshot_owned_by_other_account`，不下载或落地、不改变 owner、不为来源账号建立第二条同步记录；该远端条目仍可在来源账号的云端列表显示。
...
| 同一 owner 账号重复下载同 ID、同内容 | 成功，不重复写入本机快照仓库 |
| 另一账号下载同 ID（内容相同或不同） | `snapshot_owned_by_other_account`，不落地、不改变 owner、不建立第二条同步关系 |
...
清除授权或重新授权时必须先使旧 Runtime generation 失效，再安装新凭据并重建网络运行时。每一次百度 HTTP 请求都要取得与任务绑定账号和 generation 一致的请求租约；旧任务不能重新读取共享 Token 文件取得新账号凭据，也不能在 generation 变化后继续发出后续请求。旧 generation 的 Token 刷新、OAuth 回调和授权错误均采用 compare-and-apply，不能覆盖或清除新 Token；旧网络结果不得提交到新账号状态，任何目录和目录确保状态也不能跨账号继承。OAuth 成功后必须调用百度 `uinfo` 并以稳定 `uk` 识别真实账号，显示名称不参与判断；Token 按内部账号分别原子保存，最后才更新 SQLite 活跃账号。启动核验中，401/Token 无效进入等待授权，网络/429/5xx 保留账号与 Token 并等待重试，只有成功取得不同 `uk` 才按身份安全异常清空活跃状态。同账号重新授权可以恢复该账号义务，切换账号则使用独立本机命名空间，旧账号义务保持暂停。
