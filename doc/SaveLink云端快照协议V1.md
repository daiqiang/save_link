...
> 以下是 v0.5.0 当前实现。v0.6.0 已决定改为：已上云快照先持久化删除意图并立即从正常界面隐藏，再排队删除 metadata、`.ok`、ZIP，远端成功后才删除本机；详细状态机、迁移和验收标准见 `SaveLink-v0.6.0云任务调度与冲突治理方案.md`。在对应代码完成前，不得把计划行为写成已经实现。
