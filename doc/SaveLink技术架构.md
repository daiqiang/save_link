...
本文档描述 SaveLink v0.2.0 的真实技术架构。若本文档与代码不一致，以代码和测试为准，并应回头更新本文档。
...
- `savelink-core`：80 个默认测试全绿，其中 B 组 14 个保护直接恢复、无恢复前自动快照、最终校验失败回滚和历史数据兼容；F/H/M 组新增保护旧时间迁移、跨格式幂等、混合时间去重和正确淘汰。J/L 两个真实百度测试均已按需执行通过。
