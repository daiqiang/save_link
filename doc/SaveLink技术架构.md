...
- 动态发现正式接入：Tauri 管理全局单会话和 PID 生命周期，Windows 适配层使用 `ReadDirectoryChangesW` 监听标准用户数据根目录；游戏退出后等待 5 秒，再结合有界、身份相关和时间窗口受限的补查生成候选。
- 平台无关候选分析位于 core：聚合重复事件，保留未知扩展名，依据游戏身份、常见存档层级、主文件/备份对和噪声特征排序。监听事件只能作为推荐依据，候选默认不选择且不能自动绑定。
- 确认命令在后端重新验证会话、游戏、候选成员、目录存在性、宽泛范围、重复/父子嵌套及跨游戏冲突；确认后写入来源，并在全局自动备份开启时为当前游戏创建首份自动快照。

当前验证状态：

- `savelink-core`：125 项通过；J/L/Q5 三项真实环境测试默认忽略，均已在相应验收阶段按需执行通过。
- Tauri：自动上传状态、启动绑定/防重、原生监听、补查和候选确认共 29 个测试全绿。
