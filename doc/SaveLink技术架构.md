...
| 1.18 | Codex | 2026-08-31 | 准备发布 v0.5.0，统一当前版本元数据和架构文档的版本表述 |
...
| 1.26 | Codex | 2026-09-05 | 将当前实现基线版本统一为 v0.5.1；v0.6.0 设计暂不进入实现 |

## 文档用途

本文档描述 SaveLink v0.5.1 发布候选的真实技术架构。若本文档与代码不一致，以代码和测试为准，并应回头更新本文档。
...
- 添加游戏、编辑游戏、移除游戏。
- 创建快照、备注、锁定、删除快照。
- 打开任意快照详情时异步扫描当前真实存档，并以创建快照时的同一算法比较内容哈希、文件数量和总大小；结果仅在前端显示，不创建快照、不写真实存档、不修改 SQLite。只读扫描不获取可能覆盖云端网络请求的快照写操作锁，避免百度同步阻塞本地结果；前端以 15 秒超时结束异常等待。
...
- 快照时间线按本机显示区域分为普通区和锁定区：锁定区置顶，两个区域分别显示“存档N”和“锁定存档N”；锁定/解锁先只变更保护标识，十分钟维护周期再整理区域，期间向前端报告待整理状态。
- v0.4.0 DeSmuME 支持已接入：扫描 `desmume.ini`/ROM 目录、读取 NDS Header 身份、计算并缓存 ROM SHA-256、按目标 ROM 精确选择 `.dsv`，以及跨设备按目标设备 ROM 文件名恢复。
- 游戏程序识别已接入：当前用户页面只允许选择 Windows `.lnk` 或 EXE，后端保留有限安装目录扫描能力。读取到 AppID 后还必须验证 Manifest 游戏名与快捷方式、EXE 或目录身份至少有一项完整相等；名称冲突时忽略 AppID。无可靠 AppID 时同样只接受完整名称相等；手动添加继续保留。
...
- 动态发现正式接入：Tauri 管理全局单会话和 PID 生命周期，Windows 适配层使用 `ReadDirectoryChangesW` 监听标准用户数据根目录，并通过公共文档 Known Folder 条件性监听实际存在的 `Steam\RUNE`/`Steam\CODEX` 模拟器根目录；游戏退出后等待 5 秒，再结合有界、身份相关和时间窗口受限的补查生成候选。
- 已配置普通游戏的“启动游戏”调用只复用 `games.launch_binding` 启动 EXE 或受控 Steam 命令，不经过发现管理器的监听启动流程；子进程独立运行，动态发现状态保持不变。自动备份仍由 Tauri 启动检查和 10 分钟轮询负责。
- 平台无关候选分析位于 core：聚合重复事件，保留未知扩展名，依据游戏身份、常见存档层级和噪声特征排序。`.bak` 与主文件配对只提供辅助加分，不能单独形成领域证据；浏览器用户资料目录强制为低可信且不可确认。对精确监测的 `Steam\RUNE`/`Steam\CODEX` 根，若事件路径紧跟纯数字 AppId，则所有后代文件统一归入 `<AppId>` 候选；该规则不适用于未知根目录。监听事件只能作为推荐依据，候选默认不选择且不能自动绑定。
- 确认命令在后端重新验证会话、游戏、候选成员、目录存在性、宽泛范围、重复/父子嵌套及跨游戏冲突；确认后写入来源，并在全局自动备份开启时为当前游戏创建首份自动快照。
...
- 旧设置对话框的“打开”目录代码路径曾通过绿色版实机验证；当前齿轮入口展示新的自动备份设置页。游戏程序识别已完成代码接线及真实学习版快捷方式、EXE 窗口验收；安装目录扫描仅保留为后端能力和历史验收覆盖。
...
| 文件/目录选择 | `tauri-plugin-dialog` | 选择游戏快捷方式、EXE、DeSmuME/ROM 目录或存档目录 |
...
程序识别位于 `savelink-core/src/program_discovery.rs`：当前 UI 只提交 EXE 或 Windows `.lnk`，`.lnk` 通过 Shell COM 在独立 STA 线程读取目标；后端仍能直接解析目录，供内部测试和未来入口复用。目录扫描严格限制在安装目录 3 层和最多 512 个目录，跳过目录链接，不遍历其他磁盘。身份优先读取 `steam_appid.txt` 及常见 AppID INI，但 AppID 对应的 Manifest 游戏名必须与快捷方式名、EXE 名或目录名至少有一项在归一化后完整相等；否则将该 AppID 视为不可靠并忽略对应规则。没有可靠 AppID 时也只接受与 Manifest 游戏名完整相等，不接受前缀关系。游戏身份确定后调用 `steam_discovery.rs` 的同一套 Manifest 查询、约束过滤、占位符展开和路径收敛逻辑；未匹配时返回空结果而不是猜测。

真实样本 `HowManyDudes.exe` 携带的 AppID 会指向 Manifest 中的 `DrainSim`，但程序身份名称不相等，因此现在会忽略错误 AppID 并登记为待发现游戏。该规则由 N 组 14 个测试覆盖，包括 AppID 名称冲突和与名称一致的正常 AppID 两个方向。
...
- `Snapshot`
  ...
  - `locked`
  - `display_zone`：仅本机时间线使用的 `normal` / `locked` 显示区域，不进入云端快照协议；旧数据库打开时为已锁定记录迁入 `locked` 区。
  - `file_count`
  - `total_size`
  - `content_hash`
  - `storage_key`
...
- `save_activity.rs` 内部单元测试 16 个，覆盖事件聚合、候选评分、RUNE/CODEX 归并、浏览器资料目录不可确认和 `.bak` 辅助信号；加上其他 4 个内部单元测试，core 内部单元测试当前共 20 个。
- O 组 7 个测试：多目录指纹、根目录边界、整体恢复、旧库迁移、路径重叠防护及多来源本地一致性比较。
- P 组 3 个测试：共享 `Battery` 目录中只读取和恢复目标 `.dsv`，其他游戏存档保持不变且不影响本地一致性结果。
...
每个游戏的手动、自动、云端接收未锁定快照统一计入设置中的保留上限，默认 10、允许 1 到 100。锁定快照不计入且数量不限。删除状态分层如下：

显示区域与保护标识分离：

- `Snapshot::locked` 是自动清理保护状态，点击锁定/解锁时立即持久化。
- `Snapshot::display_zone` 是本机时间线的区域状态，点击锁定/解锁时暂不立即变更。
- `Normal + locked=true` 表示“已锁定，待整理”；`Locked + locked=false` 表示“已解锁，待整理”。
- 每十分钟维护周期将已锁定快照移入锁定区，将已解锁快照按创建时间放回普通区；普通区超出当前 N 的未锁定快照仍交给下面的云端感知联合清理流程处理。
- 自动备份关闭时不执行变化扫描和自动创建，但仍执行维护周期，因此区域整理、保留清理和云端删除重试不会因关闭创建开关而停摆。
...
当前齿轮入口渲染 `SettingsDialog`，通过 `get_auto_backup_settings` / `set_auto_backup_enabled` / `set_auto_backup_retention` 读取和修改全局自动备份开关及快照保留数量。默认值由 Tauri 初始化时写入 `app_settings`：自动备份开启、保留数量 10；设置页同时展示固定的 10 分钟检查间隔。新安装直接执行默认保留策略；已有游戏的旧数据库会先保留超出快照，用户确认保存后才触发联合清理，不需要数据库表结构迁移。
...
- v0.4.0 已正式发布；程序识别已完成真实学习版快捷方式和 EXE 窗口验收，历史安装目录入口也有回归证据，但当前 UI 已移除。名称前缀误判和错误 AppID 误命中均已修复。
...
- 动态发现无法把 Windows 目录变化严格归因到目标 PID，因此只做候选排序并要求用户确认；补查范围和事件量均有硬上限，不能退化为全盘扫描。
- 浏览器资料目录即使在游戏会话期间被后台写入，也不会成为可确认候选；`.bak` 配对不能单独证明目录属于游戏。
- `HowManyDudes` 的主存档与游戏自带 `Backup` 镜像目录都可能成为高可信候选，当前要求用户核对并只选择主存档；这是后续排序或提示增强项，不能用通用父子目录规则自动删除候选。
...
| 阶段 5 DeSmuME | ROM 发现、身份匹配、精确 `.dsv` 快照/恢复和云身份兼容已实现并随 v0.4.0 发布；Yuzu 后置到后续版本 |
| 阶段 6 游戏程序识别 | `.lnk`/EXE 身份、错误 AppID 回退和 Manifest 存档候选已实现；目录扫描保留在后端，自动测试及真实学习版样本只读验收通过 |
