...
- 平台无关候选分析位于 core：聚合重复事件，保留未知扩展名，依据游戏身份、常见存档层级、主文件/备份对和噪声特征排序。对精确监测的 `Steam\RUNE`/`Steam\CODEX` 根，若事件路径紧跟纯数字 AppId，则所有后代文件统一归入 `<AppId>` 候选；该规则不适用于未知根目录。监听事件只能作为推荐依据，候选默认不选择且不能自动绑定。
...
- Tauri：自动上传状态、启动绑定/防重、原生监听、补查和候选确认共 31 个测试全绿。
