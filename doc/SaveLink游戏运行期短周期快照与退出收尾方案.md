...
| 1.1 | Codex | 2026-08-31 | 记录本地短周期与云端维护拆分原则，以及下个版本的云任务排队方向和待确认问题 |
...
当前十分钟周期把本地变化检查、快照区域整理、云元数据同步、待处理自动上传和联合清理串在同一轮维护中。云端部分执行时，用户手动上传会被互斥保护拒绝。2026-08-31 已先完成当前版本的提示修正：后端区分“后台云同步”和“用户云操作”，并且只在后端受理手动上传后显示“正在打包并上传”。

用户决定：**自动排队不在当前版本继续实现，放到下个版本；正式开发时再确认详细产品行为。**

下个版本开始前应先拆分调度职责：

```text
游戏运行期约两分钟：只处理已确认存档的本地变化和内容去重
发现新快照：触发云端上传
每十分钟：继续执行云元数据维护、失败重试和联合清理
```

不能简单把现有 `AUTO_BACKUP_INTERVAL_MINUTES` 从 10 改为 2，否则云元数据读取、自动上传和清理频率也会提高约五倍，手动操作冲突概率随之上升。拆分只能缩短冲突窗口，不能完全消除；真正消除用户可见的冲突，需要统一云任务协调器。

建议的第一版队列边界：

- 队列只存在于 Tauri 内存，不新增 SQLite 表，不在应用重启后恢复尚未开始的任务。
- 百度上传、发现、接收、元数据同步和删除继续串行，避免并发修改同一云端对象。
- 用户发起的任务优先于尚未开始的后台任务；已发出的百度 HTTP 请求不强行中断。
- 后台批次在每条网络操作之间检查用户队列，存在用户任务时主动让行，避免用户等待整轮维护。
- 相同快照的重复上传合并或拒绝，不能产生两条实际上传。
- UI 至少区分 `等待云端任务` 和 `上传中`；排队时不显示已经开始打包。
- 应用退出时取消未开始任务；已开始任务的中断继续依靠现有云状态和下次维护重试。

第一版不做持久化任务中心、手动排序、暂停/继续、取消正在传输的 HTTP 请求，也不允许这个队列反向阻塞本地快照创建。若真实使用证明内存队列仍不足，再单独评估持久化。
...
8. 用户云任务最多允许等待多久，超时后是继续排队还是提示用户重试。
9. 多个用户任务是否严格先进先出，还是上传、接收、发现具有不同优先级。
10. 相同快照重复点击上传时，采用静默合并还是提示“已在队列中”。
11. 关闭窗口到托盘是否继续队列；只有显式退出应用时才取消，还是窗口关闭也取消。
12. UI 只在原按钮显示等待状态，还是增加全局云任务状态入口。
