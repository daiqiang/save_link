...
2026-09-02 已确认：**v0.6.0 先正式引入统一云任务协调器，再继续短周期功能。** 权威设计已迁移到 `SaveLink-v0.6.0云任务调度与冲突治理方案.md`，并确定同快照 FIFO 串行、不同快照有限并发、用户任务优先、后台 aging、共享 Token single-flight、可靠删除和未读提醒。本节以下只保留短周期功能与调度器的关系，不再作为队列实现依据。
...
短周期功能后续只负责在内容变化时创建本机快照并提交上传任务，不直接执行网络批次，也不把十分钟维护整体再触发一次。队列采用 Tauri 内存调度，跨重启可靠性由 SQLite 中的上传、metadata 和删除业务状态恢复；不允许队列反向阻塞本地快照创建。
...
8. 云任务等待、优先级、重复提交、托盘生命周期和第一版 UI 边界已在 `SaveLink-v0.6.0云任务调度与冲突治理方案.md` 中确定，不再是本方案的未决项。
